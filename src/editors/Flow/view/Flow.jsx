...
export const Flow = (props, ref) => {
  ...
  const addNodeMenu = useCallback(
    (node, nodeSelection) => {
      ...
      if (!node || !MenuComponent) return;
      drawerSub.add(
        node.data.id + "-" + MENUS.current.NODE.NAME,
  ...
  const addLinkMenu = useCallback(
    (link, _linkSelection) => {
      if (!link) return;
      drawerSub.add(
        link.data.id + "-" + MENUS.current.LINK.NAME,
  ...
  const renderMenus = useCallback(() => {
    ...
    if (selectedNodeRef.current)
      drawerSub.add(
        ...
        getNodeMenuToAdd(selectedNodeRef.current),
      );
    ...
    if (selectedLinkRef.current)
      drawerSub.add(
        ...
        getLinkMenuToAdd(selectedLinkRef.current),
      );
  }, [
    id,
    name,
