...
describe("BaseLink", () => {
  ...
  it("should render path correctly", () => {
    ...
    expect(baseLink.object.attr).toHaveBeenCalledWith(
      "id",
      "path-testCanvas-link1",
    );
  });
  ...
  it("should handle events correctly", () => {
    baseLink.renderPath();
    baseLink.addEvents();
    expect(baseLink.path.on).toHaveBeenCalledWith(
      "mouseover",
      expect.any(Function),
    );
    expect(baseLink.path.on).toHaveBeenCalledWith(
  ...
  it("should fade other links and remove fade correctly", () => {
    ...
    expect(canvas.events.next).toHaveBeenCalledWith({
      name: "onMouseOver",
      type: "Link",
      data: baseLink,
    });
